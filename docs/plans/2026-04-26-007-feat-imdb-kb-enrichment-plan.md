...
- **Confidence threshold re-calibration** — locked at 15 % per plan 004 Key Decisions. After Unit 3 runs against ~10 real releases, revisit if the fallback rate is too high or too aggressive. Threshold lives as a single module-level constant for one-line tuning. **Resolved 2026-04-26 by plan 008 (`docs/plans/2026-04-26-008-fix-imdb-vote-tie-breaker-plan.md`)** — the calibration gap surfaced via real-data verification (Roger Rabbit + Mario Galaxy multi_tie despite 100×+ vote-spread asymmetry vs the same-name catalog noise). Plan 008 added a Unit-3-layer vote-spread tie-breaker (`TIE_BREAK_VOTE_RATIO=10`, same-year guard) that demotes `multi_tie → resolved` when popularity dominance is overwhelming. The 15 % score-gap threshold inside `imdb_lookup` stays unchanged; the override runs at the consumer layer (`imdb_kb_enrich`) without breaching Unit 2's locked contract.
