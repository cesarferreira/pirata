...
- **KB enrichment consumer reality** (RBP item from origin doc) — verified 2026-04-24 via `mcp__knowledge-hub__health` + `list_kbs` + `topology`. Findings:
  - knowledge-hub is healthy on Dante (M4 primary, v0.1.0, profile `local_full_power_plus`). 34 KBs registered.
  - **Pirata `kb/` is NOT currently a registered KB.** Watched roots are all under `/Users/vidigal/knowledge-base/`; pirata lives at `/Users/vidigal/claude-code/pirata/kb/`. To become a consumer, `mcp__knowledge-hub__ingest_sync` must explicitly register the directory (one-time setup).
  - **Chunking strategy is markdown-oriented** (`target_word_range: [160, 260]`, `overlap_words: 35`). The per-movie JSONs would ingest as text-fallback — searchable as prose, not as structured records.
  - **Per-document filtering on user-defined JSON fields is NOT exposed in the retrieve surface.** Filters are score-based (`min_vector_score`, `min_lexical_overlap`, `min_combined_score`). The `factual_memory` plane has "semantic+metadata" retrieval but is a separate plane and was not designed for arbitrary user schemas.
  - **What this means for R12:** the enriched fields (`tconst`, `imdb_rating`, `genres[]`, `top_cast[5]`, `akas{pt,en,es}`) become **searchable as text** (a query containing "tt15239678" or "denis villeneuve" or "ficção científica" will hit the doc) but are **NOT filterable as structured fields** with the current knowledge-hub surface. `imdb_lookup_confidence` is debug metadata in Phase 1, not a runtime filter prop.
  - **R12 ships as schema-only prep + searchable-text enrichment**, not as filter-and-rerank substrate. The compounding-value claim narrows from "RAG can filter on tconst" to "RAG can find the doc via canonical title or person name embedded as text". Real filter-and-rerank requires Phase 2 design (separate retrieval layer, OR knowledge-hub schema work, OR migration to `factual_memory` plane). This is honest and acceptable for Phase 1; the brainstorm's reversibility framing handles it.
...
- **knowledge-hub registration** — a one-time `mcp__knowledge-hub__ingest_sync` call against `kb/` folds pirata into the hub. Listed as a Phase 1 deliverable (Unit 6 op-step), not as Phase 0 — it depends on enriched JSONs existing.

### Resolve Before Phase 1

All Resolve-Before-Phase-1 items resolved during planning (knowledge-hub schema verification + score formula + fixture target). The 15 % R5 confidence threshold calibration is moved to Deferred to Implementation (it requires the Phase 0 ingest to exist before it can be exercised against the PT-BR fixture).
...
- **R5 confidence threshold calibration** — locked at 15 % as a default proposal in Key Decisions, but never exercised against real titles. Run the 20-item PT-BR fixture during Unit 2 and verify R5 forces vs auto-picks match intent on multi-tie cases (e.g., aka collisions like `O Iluminado`). Adjust the percentage in `imdb_lookup.py` if the default surfaces unexpected silent picks. This calibration depends on the Phase 0 ingest existing — runs once, locks for Phase 1.
...
- [ ] **Unit 6: Operations — `.gitignore`, `imdb/state.json` stub, `imdb/README.md`, `scripts/pirata_evaluate.py`, smoke tests**
  ...
  **Approach:**
  ...
  - **Knowledge-hub registration op-step** (Phase 1 only, after first sweep with `--kb-imdb` produces enriched JSONs): one-time `mcp__knowledge-hub__ingest_sync` call against `kb/` (or a documented manual command in `imdb/README.md`) registers pirata as a hub-watched directory. The pirata `kb/` is currently NOT in the hub's watched roots (verified 2026-04-24); without this registration, the enriched JSONs are local-only files. After registration, hub queries find the docs by canonical title or person name (text-search); structured filtering (`tconst = ?`, `genres ⊃ ?`) is NOT supported by the current retrieve surface and lands as a deferred Phase-2 design item if it ever becomes a felt need.
